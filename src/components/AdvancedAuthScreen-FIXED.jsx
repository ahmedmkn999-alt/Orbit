...
export default function AdvancedAuthScreen() {
  ...
  const handleRequestOTP = async () => {
    ...
    if (result.success) {
      ...
      setSentToTelegram(result.sentToTelegram);
      
      if (result.isNewUser) {
        // New user - go straight to OTP + Password + Profile
        setLoginMethod('otp');
        setStep(3);
      } else {
        // Existing user - show login method choice
        setStep(2);
      }
    } else {
  ...
  const handleVerifyOTP = async () => {
    ...
    if (isNewUser) {
      // For new users, go to profile setup with password
    ...
    const result = await verifyOTP(phoneNumber, cleanedOtp, null, false, null);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-orbitSpace p-4">
      <div className="glass-panel w-full max-w-md p-8 rounded-3xl border border-orbitCyan/30 shadow-neonCyan">
        ...
        {step === 3 && (
          <div className="space-y-6">
            {/* OTP Display for New Users */}
            {isNewUser && generatedOTP && (
            ...
            {loginMethod === 'otp' ? (
              ...
              <>
                ...
                <button 
                  ...
                  className="w-full text-center text-sm text-orbitCyan hover:text-orbitPurple transition-colors"
                >
                  نسيت كلمة المرور؟
                </button>
              </>
            ...
            <button 
              onClick={() => { setStep(isNewUser ? 1 : 2); setOtp(''); setPassword(''); setConfirmPassword(''); setError(''); }}
              className="w-full text-center text-sm text-gray-400 hover:text-orbitCyan transition-colors flex items-center justify-center gap-1"
            >
        ...
        {step === 5 && (
          <div className="space-y-6">
            <p className="text-sm text-gray-400 mb-3 text-right">
              تم إرسال كود الاستعادة إلى بوت التيليجرام
            </p>
            ...
            <button 
              onClick={() => { setStep(1); setOtp(''); setPassword(''); setConfirmPassword(''); setError(''); }}
              className="w-full text-center text-sm text-gray-400 hover:text-orbitCyan transition-colors flex items-center justify-center gap-1"
            >
              <ArrowRight size={14} /> رجوع للبداية
            </button>
          </div>
        )}
      </div>
