...
export default function AdvancedAuthScreen() {
  ...
  const handleVerifyOTP = async () => {
    ...
    // التحقق نجح - نشوف هل المستخدم موجود في Firestore ولا لا
    const result = await verifyOTP(phoneNumber, cleanedOtp, null, false, null);

    if (result.success) {
      // مستخدم موجود - روح للهوم
      window.location.href = '/home';
    } else {
    ...
    <div className="min-h-screen flex items-center justify-center bg-orbitSpace p-4">
      <div className="glass-panel w-full max-w-md p-8 rounded-3xl border border-orbitCyan/30 shadow-neonCyan">
        ...
        <div style={{
          ...
          <div style={{
            margin: '16px', background: 'rgba(239,68,68,0.95)', backdropFilter: 'blur(8px)',
            color: 'white', padding: '12px 20px', borderRadius: '16px',
            boxShadow: '0 4px 20px rgba(0,0,0,0.4)', fontSize: '14px',
            textAlign: 'right', maxWidth: '360px', width: '100%'
          }}>
            {toast}
          </div>
        </div>
